...
async function runStore(browser, store) {
  ...
  try {
    ...
    const difference = previousNumeric && currentNumeric ? currentNumeric - previousNumeric : '';
...
async function startCheckoutFromStore(page, storeUrl, configuredProductUrl) {
  ...
  // First try to collect candidates from the current homepage and common listing pages.
  const candidates = await collectProductCandidates(page, storeUrl);
  if (!candidates.length) {
    // Fallback to older single-product finder.
...
async function tryProductAndStartCheckout(page, productUrl, origin) {
  ...
  // Preferred path: product page Buy Now.
  ...
  // Fallback path: Add to cart, then checkout/cart.
  ...
  // Direct URL fallback for common platforms.
  await page.goto(`${origin}/checkout`, { waitUntil: 'domcontentloaded', timeout: 60000 }).catch(async () => {
...
async function extractCandidateLinks(page, origin) {
  ...
  for (const link of rawLinks) {
    try {
      ...
      if (score >= 9) candidates.push({ url: clean, score });
    } catch (_) {}
  }
  return candidates;
}
...
async function selectProductOptions(page) {
  // 1) Native selects: choose the first real enabled option, ignoring placeholders like "اختر".
  ...
  // 2) Salla/common option buttons/swatches/radio labels: click the first available option in each detected group.
...
function isActionText(text) {
  ...
  const actions = [...addToCartTexts, ...buyNowTexts, ...checkoutTexts, ...placeOrderTexts, /الرئيسية/i, /القائمة/i, /بحث/i, /search/i];
...
async function isProductUnavailable(page) {
  ...
  // If all visible buy/cart buttons are disabled, treat as unavailable.
...
async function logResult(result) {
  const res = await fetch(WEBAPP_URL, {
    ...
    body: JSON.stringify({ action: 'log_result', result })
  });
...
async function clickPlaceOrder(page) {
  // 1) Try normal visible text / accessible name first.
  if (await clickByTexts(page, placeOrderTexts, 'Place order', { optional: true })) return true;

  // 2) Many ecommerce checkouts use input[type=submit] where the label is in value/name/id, not innerText.
  ...
  // 3) WooCommerce / common checkout selectors.
  ...
  // 4) Last fallback: click the last visible submit button in the main checkout area.
...
async function dismissPopups(page) {
  const patterns = [/accept/i, /agree/i, /close/i, /قبول/i, /موافق/i, /اغلاق/i, /إغلاق/i, /x/i];
  for (const pattern of patterns) {
    try {
